        ...
        <>
          ...
          {showForm && (
            <div className="bg-[rgba(17,17,34,0.6)] backdrop-blur-xl border border-[rgba(255,255,255,0.06)] rounded-2xl p-5 animate-slideUp">
              <h2 className="text-sm font-semibold text-[#D4A843] mb-4">إضافة مكالمة جديدة</h2>
              <div className="space-y-3">
                <div className="flex gap-3">
                  ...
                  <input value={form.caller_name} onChange={e => setForm({...form, caller_name: e.target.value})} placeholder="اسم المتصل" className="flex-1 px-4 py-3 rounded-xl bg-[#13131A] border border-[#1F1F2A] text-white placeholder-gray-500 focus:outline-none focus:border-[#D4A843] transition-all" />
                </div>
                <div className="flex gap-3">
                  <input value={form.caller_number} onChange={e => setForm({...form, caller_number: e.target.value})} placeholder="رقم الهاتف" className="flex-1 px-4 py-3 rounded-xl bg-[#13131A] border border-[#1F1F2A] text-white placeholder-gray-500 focus:outline-none focus:border-[#D4A843] transition-all" />
                  <input value={form.duration_seconds} onChange={e => setForm({...form, duration_seconds: e.target.value})} placeholder="المدة (ثواني)" type="number" className="w-40 px-4 py-3 rounded-xl bg-[#13131A] border border-[#1F1F2A] text-white placeholder-gray-500 focus:outline-none focus:border-[#D4A843] transition-all" />
                </div>
                <textarea value={form.summary} onChange={e => setForm({...form, summary: e.target.value})} placeholder="ملخص المكالمة" rows={2} className="w-full px-4 py-3 rounded-xl bg-[#13131A] border border-[#1F1F2A] text-white placeholder-gray-500 focus:outline-none focus:border-[#D4A843] transition-all" />
          ...
          {logs.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-center bg-[rgba(17,17,34,0.6)] backdrop-blur-xl border border-[rgba(255,255,255,0.06)] rounded-2xl">
              ...
              <button onClick={() => setShowForm(true)} className="px-5 py-2.5 rounded-xl font-semibold text-sm bg-gradient-to-r from-[#D4A843] to-[#e4b84a] text-[#0A0A0F] transition-all">إضافة مكالمة</button>
            </div>
          ) : (
            <div className="space-y-3">
              {logs.map(log => (
                    ...
                    <div className="flex items-start gap-3 flex-1 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="flex items-center gap-3 mt-2">
                          ...
                          {log.recording_path && (
                            <span className="text-[11px] text-[#D4A843] flex items-center gap-1">🔊 تسجيل</span>
                          )}
                          {log.created_at && (
