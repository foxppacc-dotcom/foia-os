...
import { MessageSquare, Send, Paperclip, Link2, FileText, X, User, ExternalLink, Reply, Trash2, AtSign } from 'lucide-react';
...
// Team discussion / notes on the case -- distinct from the auto-generated
// system entries (case created, classified, document uploaded...) that also
// live in case_comments; this is where humans actually talk to each other
// about the case, right in the Overview tab instead of buried in the
// Timeline's mixed system+human feed.
export default function TeamDiscussion() {
  const { id: caseId, comments: allComments, users, refetch } = useCaseContext();
  // This table also holds each checklist item's own notes thread
  // (record_type set) -- the general "نقاش الفريق" only ever shows/replies
  // within the unscoped (record_type null) thread.
  const comments = (allComments || []).filter(c => !c.record_type);
  ...
  // Re-render every few seconds so the "delete own comment" window (60s)
  // actually disappears from the UI once it expires, instead of only
  // updating on the next unrelated refetch.
  ...
  // Sourced from the org-wide employee list, not just this case's formally
  // assigned team (case_assignees) -- a case with no assignees yet (or a
  // colleague not formally added) previously left the mention picker empty,
  // which looked like the feature simply didn't work.
  ...
  return (
    <div className="rounded-xl border p-4" style={{ background: 'var(--ds-bg-secondary)', borderColor: 'var(--ds-border)', boxShadow: 'var(--ds-shadow-sm)' }}>
      <div className="flex items-center gap-2 mb-3">
        ...
        <h3 className="text-sm font-semibold" style={{ color: 'var(--ds-text-primary)' }}>نقاش الفريق ({comments?.length || 0})</h3>
      </div>

      {/* Composer */}
      <div className="space-y-2 mb-4 p-3 rounded-lg" style={{ background: 'var(--ds-bg-tertiary)' }}>
        {replyTo && (
        ...
        <textarea value={text} onChange={e => setText(e.target.value)} rows={2}
          placeholder="اكتب ملاحظة أو تعليق للفريق..."
      ...
      {(!comments || comments.length === 0) ? (
        <p className="text-xs text-center py-4" style={{ color: 'var(--ds-text-muted)' }}>لا توجد ملاحظات بعد — ابدأ النقاش</p>
      ) : (
