    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-fadeIn" style={{ background: 'var(--bg-overlay)' }} onClick={onClose}>
      <div className="relative max-w-3xl max-h-[90vh] w-full animate-scaleIn" onClick={e => e.stopPropagation()}>
        ...
        <div className="rounded-2xl overflow-hidden bg-black" style={{ border: '1px solid rgba(255,255,255,0.1)' }}>
          {file.type === 'image' ? (
            <img src={file.isUrl ? file.path : '/' + file.path} alt={file.name} className="max-w-full max-h-[80vh] mx-auto" style={{ objectFit: 'contain' }} />
          ) : file.type === 'video' ? (
            <video controls className="w-full max-h-[80vh]" src={file.isUrl ? file.path : '/' + file.path} />
          ) : file.type === 'audio' ? (
            <div className="flex flex-col items-center justify-center p-12">
              <p className="text-white text-sm mb-4">{file.name}</p>
              <audio controls src={file.isUrl ? file.path : '/' + file.path} className="w-full max-w-md" />
            </div>
          ) : file.type === 'pdf' ? (
            <div className="w-full h-[80vh] bg-white">
              <iframe src={file.isUrl ? file.path : '/' + file.path} title={file.name} className="w-full h-full border-0" />
            </div>
          ) : (
            <div className="flex flex-col items-center justify-center gap-3 p-12">
              <p className="text-white text-sm">{file.name}</p>
              <a href={file.isUrl ? file.path : '/' + file.path} target="_blank" rel="noopener noreferrer"
              ...
              <button onClick={() => window.open(file.isUrl ? file.path : '/' + file.path, '_blank')}
          ...
          <p className="text-center text-xs text-gray-400 pb-4">{file.name}</p>
        </div>
      </div>
    </div>
  );
